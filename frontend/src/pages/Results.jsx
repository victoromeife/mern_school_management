...
const Results = () => {
  ...
  // Fetch student results when selected student changes (parent role)
  useEffect(() => {
    if (selectedStudent) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      {/* Stats Cards for student/parent */}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
          <h2 className="text-lg font-semibold text-surface-900 dark:text-white">
            {user?.role === 'student'
              ? 'Your Results'
              : user?.role === 'parent'
              ? `${students.find((s) => s._id === selectedStudent)?.name}'s Results`
              : 'All Results'}
          </h2>
